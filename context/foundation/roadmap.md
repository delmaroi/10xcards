...
| ---- | ----------------------------- | ----------------------------------------------------- | ------------- | ---------------------- | -------- |
| F-01 | gate-product-routes           | (foundation) product routes behind login + landing    | —             | FR-001, FR-002         | ready    |
| F-02 | minimal-flashcard-persistence | (foundation) per-user flashcard store with isolation  | —             | FR-007                 | ready    |
| F-03 | srs-library-spike             | (foundation) chosen SRS library contract verified     | —             | FR-012                 | ready    |
| S-01 | first-gated-generation        | paste text and triage AI card proposals               | F-01, F-02    | US-01, FR-005, FR-006  | proposed |
| S-02 | atomic-save-to-deck           | finalize accepted cards into the deck                 | S-01          | US-01, FR-007, FR-009  | proposed |
| S-03 | deck-edit-delete              | browse, edit and delete saved cards                   | S-02          | FR-009, FR-010, FR-011 | proposed |
| S-04 | success-metrics-collection    | admin can see AI acceptance-rate and AI-share metrics | S-02          | FR-004                 | proposed |
| S-05 | srs-review-session            | run a review session and record a card rating         | S-02, F-03    | US-01, FR-012          | blocked  |
...
| C      | Review loop     | `F-03` → `S-05`                            | Joins Stream A at `S-02`; blocked until the SRS choice lands. |
...
- **Risk:** Completes the full learning thesis, but the ready-made-algorithm contract dictates the ReviewState model and rating flow. Blocked until F-03 resolves the library choice — exactly the kind of unknown that should surface on the roadmap, not mid-implementation.
- **Status:** blocked
...
| ---------- | ----------------------------- | -------------------------------------------------- | --------------------- | --------------------------------------------- |
| F-01       | gate-product-routes           | Gate product routes behind login + landing page    | yes                   | Run `/10x-plan gate-product-routes`           |
| F-02       | minimal-flashcard-persistence | Minimal per-user flashcard persistence + RLS       | yes                   | Run `/10x-plan minimal-flashcard-persistence` |
| F-03       | srs-library-spike             | Spike: verify SRS library contract                 | yes                   | Resolves Open Question 4                      |
| S-01       | first-gated-generation        | First gated AI generation loop (paste → triage)    | no                    | Needs F-01, F-02 first                        |
| S-02       | atomic-save-to-deck           | Atomic save of accepted cards to deck (north star) | no                    | Needs S-01 first                              |
| S-03       | deck-edit-delete              | Browse, edit and delete saved cards                | no                    | Needs S-02 first                              |
| S-04       | success-metrics-collection    | Collect AI acceptance-rate & AI-share metrics      | no                    | Needs S-02 first                              |
| S-05       | srs-review-session            | Spaced-repetition review session                   | no                    | Blocked on F-03 (SRS choice)                  |
