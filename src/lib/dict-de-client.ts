...
/**
 * Call GET /api/dict/de/{word} and return the normalized result. An empty
 ...
 */
export async function lookupWordDeClient(word: string): Promise<DictionaryLookupResult> {
  let res: Response;
  try {
    res = await fetch(`/api/dict/de/${encodeURIComponent(word)}`, {
      credentials: "include",
    });
  } catch {
